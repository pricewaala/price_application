import '../check_out_one_screen/widgets/productcard5_item_widget.dart';
import 'bloc/check_out_one_bloc.dart';
import 'models/check_out_one_model.dart';
...
class CheckOutOneScreen extends StatelessWidget {
  ...
  Widget _buildShippingForm(BuildContext context) {
    return BlocSelector<CheckOutOneBloc, CheckOutOneState,
        TextEditingController?>(
      selector: (state) => state.shippingFormController,
      builder: (context, shippingFormController) {
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            BlocSelector<CheckOutOneBloc, CheckOutOneState, CheckOutOneModel?>(
              selector: (state) => state.checkOutOneModelObj,
              builder: (context, checkOutOneModelObj) {
                return CustomDropDown(
                  hintText: "lbl_country".tr,
                  hintStyle: CustomTextStyles.bodyMediumBluegray40001,
                  items: checkOutOneModelObj?.dropdownItemList ?? [],
                  onChanged: (value) {
                    context
                        .read<CheckOutOneBloc>()
          ...
          child: BlocSelector<CheckOutOneBloc, CheckOutOneState,
            ...
            builder: (context, checkOutOneModelObj) {
              return ListView.separated(
                ...
                itemCount:
                    checkOutOneModelObj?.productcard5ItemList.length ?? 0,
                itemBuilder: (context, index) {
                  Productcard5ItemModel model =
                      checkOutOneModelObj?.productcard5ItemList[index] ??
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: BlocSelector<CheckOutOneBloc, CheckOutOneState, bool?>(

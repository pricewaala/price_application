import '../product_full_one_screen/widgets/productcardlist_item_widget.dart';
import '../product_full_one_screen/widgets/view3_item_widget.dart';
import 'bloc/product_full_one_bloc.dart';
import 'models/product_full_one_model.dart';
...
class ProductFullOneScreen extends StatelessWidget {
  const ProductFullOneScreen({Key? key})
      : super(
          key: key,
        );

  static Widget builder(BuildContext context) {
    return BlocProvider<ProductFullOneBloc>(
      create: (context) => ProductFullOneBloc(ProductFullOneState(
        productFullOneModelObj: ProductFullOneModel(),
      ))
        ..add(ProductFullOneInitialEvent()),
      child: ProductFullOneScreen(),
    );
  }
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        ...
        bottomNavigationBar: _buildComponentTwoColumn(context),
      ),
    );
  }

  /// Section Widget
  Widget _buildProductStack(BuildContext context) {
    return Align(
      alignment: Alignment.topRight,
      child: SizedBox(
        height: 509.v,
                    ...
                    Align(
                      ...
                      child: SizedBox(
                        ...
                        child: Stack(
                          alignment: Alignment.bottomCenter,
                          children: [
                            BlocBuilder<ProductFullOneBloc,
                                ProductFullOneState>(
                              builder: (context, state) {
                                return CarouselSlider.builder(
                                  ...
                                  itemCount: state.productFullOneModelObj
                                          ?.view3ItemList.length ??
                                      0,
                                  itemBuilder: (context, index, realIndex) {
                                    View3ItemModel model = state
                                            .productFullOneModelObj
                                            ?.view3ItemList[index] ??
                                        View3ItemModel();
                                    return View3ItemWidget(
                                      model,
                                    );
                                  },
                                );
                            ...
                            Align(
                              alignment: Alignment.bottomCenter,
                              child: BlocBuilder<ProductFullOneBloc,
                                  ProductFullOneState>(
                                builder: (context, state) {
                                  return SizedBox(
                                    height: 10.v,
                                    child: AnimatedSmoothIndicator(
                                      ...
                                      count: state.productFullOneModelObj
                                              ?.view3ItemList.length ??
                                          0,
  ...
  /// Section Widget
  Widget _buildFortyNineRow(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(
        left: 1.h,
        right: 2.h,
  ...
  Widget _buildProductCardList(BuildContext context) {
    return Align(
      ...
      child: SizedBox(
        height: 1782.v,
        child: BlocSelector<ProductFullOneBloc, ProductFullOneState,
            ProductFullOneModel?>(
          selector: (state) => state.productFullOneModelObj,
          builder: (context, productFullOneModelObj) {
            return ListView.separated(
              ...
              itemCount:
                  productFullOneModelObj?.productcardlistItemList.length ?? 0,
              itemBuilder: (context, index) {
                ProductcardlistItemModel model =
                    productFullOneModelObj?.productcardlistItemList[index] ??
